...
export const FuncionariosPage: React.FC = () => {
  ...
  const filteredFuncionarios = useMemo(
    () =>
      funcionarios.filter((funcionario) => {
        const matchesStatus =
          statusFilter === "TODOS" ||
          funcionario.status.toUpperCase() === statusFilter;
  ...
  const editFuncionario = async (id: number, data: FuncionarioFormData) => {
    try {
      ...
      setFuncionarios((previous) =>
        previous.map((funcionario) =>
          funcionario.id === id ? updated : funcionario,
        ),
      );
    } catch (err) {
  ...
  const deleteFuncionario = async (id: number) => {
    try {
      ...
      setFuncionarios((previous) =>
        previous.filter((funcionario) => funcionario.id !== id),
      );
    } catch (err) {
